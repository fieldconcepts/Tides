...
for i in range(12):
    ...
    svg_paths = createSVG(data[0], data[1], X_SCALE_FACTOR, Y_SCALE_FACTOR,  X_FRAME_OFFSET, Y_FRAME_OFFSET, TEXT_SPACING, FONT_SIZE, TEXT_COLOUR, LINE_WEIGHT, LINE_COLOUR)
    ...
    text_grid = '<rect x="{}" y="{}" width="{}" height="{}" stroke="{}" stroke-width="{}" fill="{}"/>'.format(GAPS, y_coord, TEXT_BOX_W, BOX_HEIGHT, GRID_LINE_COLOUR, GRID_WEIGHT, GRID_COLOUR)
    
    #TODO . code for the vetical rotated month text goes here...
    
    grid = '<rect x="{}" y="{}" width="{}" height="{}" stroke="{}" stroke-width="{}" fill="{}"/>'.format(TEXT_BOX_W + (GAPS*2), y_coord, PLOT_BOX_W, BOX_HEIGHT,GRID_LINE_COLOUR, GRID_WEIGHT, GRID_COLOUR)     
